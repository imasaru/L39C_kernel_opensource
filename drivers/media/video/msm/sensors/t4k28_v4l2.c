...
static struct msm_camera_i2c_conf_array t4k28_fps_range_confs[][1] = {
	{
...
static struct msm_camera_i2c_conf_array t4k28_bestshot_mode_confs[][1] = {
	{	//SCENE OFF
		...
		//	0,		MSM_CAMERA_I2C_BYTE_DATA}, //V1 BringUP //aidan.cho@lge.com 20120314
	},
	{	//SCENE AUTO
		{&t4k28_scene_normal_settings[0], ARRAY_SIZE(t4k28_scene_normal_settings),
			10,		MSM_CAMERA_I2C_BYTE_DATA},
...
int8_t t4k28_get_snapshot_data(struct msm_sensor_ctrl_t *s_ctrl, struct snapshot_soc_data_cfg *snapshot_data) {
	int rc = 0;
	unsigned short analogGain1 = 0;//added Register setting for v1 ISO (AUTO) , aidan.cho@lge.com 2013/05/08
	unsigned short analogGain2 = 0;//added Register setting for v1 ISO (AUTO) , aidan.cho@lge.com 2013/05/08
	...
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3561,&analogGain1, MSM_CAMERA_I2C_BYTE_DATA);//added Register setting for v1 ISO (AUTO) , aidan.cho@lge.com 2013/05/08
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3562,&analogGain2, MSM_CAMERA_I2C_BYTE_DATA);//added Register setting for v1 ISO (AUTO) , aidan.cho@lge.com 2013/05/08
	...
	//Exposure Time 
	/* LGE_CHANGE_S: aidan.cho@lge.com Added Register setting for V1 */
	...
	/* LGE_CHANGE_E: aidan.cho@lge.com Added Register setting for V1*/
	...
	//ISO speed
	analogGain = ((analogGain1<<8)|analogGain2); //added Register setting for v1 ISO (AUTO) , aidan.cho@lge.com 2013/05/08
	...
	//Exposure Time
	ExposureTotal = ((Exposure1<<8)|Exposure2);// Added Register setting for V1 , aidan.cho@lge.com 2013/04/15
	//pr_err("[aidan]%s : Exposure time -  ExposureTotal = %d ",  __func__,  ExposureTotal);
	//pr_err("[aidan]%s : Exposure time -  ExposureTotal = %d ",  __func__,  ExposureTotal);
	if (ExposureTotal <= 0) {
		exposureTime = 600000;
	}else {
		exposureTime = ExposureTotal;// Added Register setting for V1 , aidan.cho@lge.com 2013/04/15
			...
			int update_type, int res)
{
	...
	int retry = 0; /*LGE_CHANGE  V1 Fps issue fixed for DV3, 2013.05.21, aidan.cho@lge.com */
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		/*LGE_CHANGE_S, LGE_CHANGE  V1 Fps issue fixed for DV3, 2013.05.21, aidan.cho@lge.com*/
		if(res == 1 && PREV_FPS != -1) // return to preview 
		{
			//printk("%s: PREV_FPS = %d\n", __func__, PREV_FPS);
			for (retry = 0; retry < 3; ++retry) {
				rc = msm_sensor_write_enum_conf_array(
				s_ctrl->sensor_i2c_client,
				&t4k28_fps_range_enum_confs, PREV_FPS);
		            
				if (rc < 0)
					printk(KERN_ERR "[ERROR]%s:write failed for :  value = %d, retry = %d\n", __func__, PREV_FPS, retry);
				else
					break;
			}
		}
		/*LGE_CHANGE_E, LGE_CHANGE  V1 Fps issue fixed for DV3, 2013.05.21, aidan.cho@lge.com*/
