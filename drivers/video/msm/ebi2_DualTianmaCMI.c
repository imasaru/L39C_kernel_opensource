...
static void panel_CMIDisplay_init(void)
{
	...
	EBI2_WRITE16C(DISP_CMD_PORT, 0xE0); //Set P Gamma //value up brightness up 
			EBI2_WRITE16D(DISP_DATA_PORT, 0x00); //0
			EBI2_WRITE16D(DISP_DATA_PORT, 0x08); //1  //08
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0E); //2  //0E
			EBI2_WRITE16D(DISP_DATA_PORT, 0x04); //4
			EBI2_WRITE16D(DISP_DATA_PORT, 0x12); //6
			EBI2_WRITE16D(DISP_DATA_PORT, 0x06); //13 
			EBI2_WRITE16D(DISP_DATA_PORT, 0x2E); //20  32
			EBI2_WRITE16D(DISP_DATA_PORT, 0x87); //36 27  67
			EBI2_WRITE16D(DISP_DATA_PORT, 0x40); //43  --3f
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0A); //50 --08
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0E); //57 --0e
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0B); //59  --0a
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0F); //61---10
			EBI2_WRITE16D(DISP_DATA_PORT, 0x13); //62
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0f); //63

	EBI2_WRITE16C(DISP_CMD_PORT, 0XE1); //Set N Gamma value down brightness down
			EBI2_WRITE16D(DISP_DATA_PORT, 0x00); //63
			EBI2_WRITE16D(DISP_DATA_PORT, 0x22); //62
			EBI2_WRITE16D(DISP_DATA_PORT, 0x23); //61
			EBI2_WRITE16D(DISP_DATA_PORT, 0x02); //59 //03
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0D); //57 //0f
			EBI2_WRITE16D(DISP_DATA_PORT, 0x02); //50 //05
			EBI2_WRITE16D(DISP_DATA_PORT, 0x39); //43  --3a
			EBI2_WRITE16D(DISP_DATA_PORT, 0x37); //27 36
			EBI2_WRITE16D(DISP_DATA_PORT, 0x50); //20 4C
			EBI2_WRITE16D(DISP_DATA_PORT, 0x04); //13
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0d); //6
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0b); //4
			EBI2_WRITE16D(DISP_DATA_PORT, 0x33); //2
			EBI2_WRITE16D(DISP_DATA_PORT, 0x36); //1  //36
   ...
   if((pdata->initialized && system_state == SYSTEM_BOOTING) || lcd_init_skip_cnt < 1) {
	...
	else 
	{
		...
		else 
		{
			...
			EBI2_WRITE16C(DISP_CMD_PORT,  0XEE);
			EBI2_WRITE16D(DISP_DATA_PORT, 0x0C);	//Prevent tearing effect
