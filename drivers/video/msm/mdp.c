...
{
	ssize_t ret = 0;
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA) && defined(CONFIG_LGE_FB_MSM_MDP_LUT_ENABLE)
	int tmp = 0;
	extern int lcd_ctrl(void);
#endif
	...
	{
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA) && defined(CONFIG_LGE_FB_MSM_MDP_LUT_ENABLE)
		printk("%s(),1  suspend:%d, vsync_resume:%d\n",
				__func__, atomic_read(&vsync_cntrl.suspend), atomic_read(&vsync_cntrl.vsync_resume));

		if((0 == atomic_read(&vsync_cntrl.vsync_resume)) && (0 == atomic_read(&vsync_cntrl.suspend)))
		{
			tmp = lcd_ctrl();
			if(!tmp)
			{
				printk("%s(),2  suspend:%d, vsync_resume:%d\n",
				__func__, atomic_read(&vsync_cntrl.suspend), atomic_read(&vsync_cntrl.vsync_resume));
			}
		}
		else
