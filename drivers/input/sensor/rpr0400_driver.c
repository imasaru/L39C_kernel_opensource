...
static int rpr400_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	disable_irq(client->irq);
	flush_delayed_work(&als_ps->dwork);
...
static int rpr400_resume(struct i2c_client *client)
{
    ...
    /*[yunmo.yang@lge.com] Sensor Init Sequence Change. INIT -> Interrupt enable, Becousse of High Temperature and Low Temperature Test */
    //enable_irq(client->irq);
	err = pdata->power(&client->dev, 1);
	mdelay(50);
	...
	enable_irq(client->irq);  /*[yunmo.yang@lge.com] Sensor Init Sequence Change. INIT -> Interrupt enable*/
