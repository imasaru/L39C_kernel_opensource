...
/* LGE_CHANGE_S  : adiyoung.lee, AAT Result on MISC, 2013-01-15 */
...
#endif
/* LGE_CHANGE_E  : adiyoung.lee, AAT Result on MISC, 2013-01-15 */
...
//LGE_S, jaekyung83.lee@lge.com, 2013-06-11, testmode 9.5 250-2-8 RGB Read CMD 
#if defined(CONFIG_MACH_MSM7X25A_V1)
#define LCD_K_CAL_SIZE 13
extern int lge_emmc_misc_read(unsigned int blockNo, char* buffer, int size);
static int read_lcd_k_cal(char* buf)
{
	int size = -1;

	size = lge_emmc_misc_read(44, buf, LCD_K_CAL_SIZE);
...
#endif
//LGE_E, jaekyung83.lee@lge.com, 2013-06-11, testmode 9.5 250-2-8 RGB Read CMD 
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
...
#if defined(CONFIG_MACH_MSM7X25A_V1)
	//LGE_S, jaekyung83.lee@lge.com, 2013-06-11, testmode 9.5 250-2-8 RGB Read CMD 
	if ((pReq->MaxRGB[0] != 5)  && (pReq->MaxRGB[0] != 8)) { //sub_cmd = 2,3,4,7
		update_diagcmd_state(diagpdev, "LCD_Cal", pReq->MaxRGB[0]);
	}
	else if(pReq->MaxRGB[0] == 8) {
		char buf[LCD_K_CAL_SIZE];
		int ret = -1;
		memset(buf, 0x00, sizeof(buf));
		ret = read_lcd_k_cal(buf);

		pRsp->ret_stat_code = TEST_OK_S;
		if(0 == strlen(buf))
		{
			sprintf(pRsp->test_mode_rsp.lcd_get_kcal,"000000000\n");
		}
		else
		{
			memset(pRsp->test_mode_rsp.lcd_get_kcal, 0x00, sizeof(char) * 10);
			memcpy(pRsp->test_mode_rsp.lcd_get_kcal,buf, sizeof(char) * 9);
		}
	}
#elif defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	//LGE_S, jaekyung83.lee@lge.com, 2013-06-11, testmode 9.5 250-2-8 RGB Read CMD 
	if ((pReq->MaxRGB[0] != 5)  && (pReq->MaxRGB[0] != 8)) { //sub_cmd = 2,3,4,7
		update_diagcmd_state(diagpdev, "LCD_Cal", pReq->MaxRGB[0]);
	}
	else if(pReq->MaxRGB[0] == 8) {
		//LGE_S, jaekyung83.lee@lge.com, 2013-07-08, testmode 9.5 250-2-8 RGB Read CMD
		if ((pReq->MaxRGB[0] != 5)  && (pReq->MaxRGB[0] != 8)) { //sub_cmd = 2,3,4,7
			update_diagcmd_state(diagpdev, "LCD_Cal", pReq->MaxRGB[0]);
		}
		else if(pReq->MaxRGB[0] == 8) {
			pRsp->ret_stat_code = TEST_OK_S;
			memset(pRsp->test_mode_rsp.lcd_get_kcal, 0x00, sizeof(char) * 10);
			sprintf(pRsp->test_mode_rsp.lcd_get_kcal, "%03d%03d%03d",g_lge_lcd_k_cal[0], g_lge_lcd_k_cal[1], g_lge_lcd_k_cal[2]);
		}
		//LGE_E, jaekyung83.lee@lge.com, 2013-07-08, testmode 9.5 250-2-8 RGB Read CMD 
	}
#else
		...
		DIAG_TEST_MODE_F_rsp_type	*pRsp)
{
	...
	switch (req_ptr.test_mode_req.key_lock_unlock)
	{
		...
		case 1:
			...
			break;
		case 2://check the key lock/unlock .
...
#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK
void lg_get_usb_access_lock_misc(void)
{
	int result = -1;
	char usb_lock_state[10] = {0,};

	result = lge_emmc_misc_read(2048*27/512,usb_lock_state,10);

#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK_DEBUG
	printk("%s :: lock_state = %s\n",__func__,usb_lock_state);
#endif
	return;
}
EXPORT_SYMBOL(lg_get_usb_access_lock_misc);

void lg_set_usb_access_lock_misc(int value)
{
	int result = -1;
	char usb_lock_state[10] = {0,};

	if (value == 0) {
		strncpy(usb_lock_state,"unlock",6);
	}
	else {
		strncpy(usb_lock_state,"lock",4);
	}

	result = lge_emmc_misc_write(2048*27/512,usb_lock_state,10);

#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK_DEBUG
	printk("%s :: lock_state ::%s result = %d\n",__func__,usb_lock_state,result);
#endif
	return;
}
EXPORT_SYMBOL(lg_set_usb_access_lock_misc);
#endif
...
#define BOOT_MODE  "/sys/devices/platform/lge_boot_mode/boot_mode"
void* LGF_TestModeMiniOsBootCheck(
                test_mode_req_type* pReq ,
                DIAG_TEST_MODE_F_rsp_type       *pRsp)
{
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
...
//	char *dest = (void *);
	...
	{
		if (!((fd = sys_open((const char __user *) BOOT_MODE, O_RDONLY, 0) ) < 0 ))
		{
			if (!((sys_read(fd, buf, 10)) < 0))
        		{
				if(strncmp(buf,"miniOS",6)==0)
				{
					return_value = TEST_OK_S;
				}
			}			
	...
	pRsp->ret_stat_code = return_value;
	return pRsp;
#else
	pRsp->ret_stat_code = TEST_NOT_SUPPORTED_S;
	return pRsp;
...
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
// LGE_S, UTS support
//#define UTS_RADIO_ON 1
//#define UTS_RADIO_OFF 4
PACK(void*) LGF_UTSRadio(PACK(void*) req_pkt_ptr, uint16 pkt_len)
{
      ...
      DIAG_UTS_RADIO_rsp_type *rsp_ptr;
       ...
       case 0: // UTS Radio On(PowerUp)
               set_operation_mode(TRUE/*UTS_RADIO_ON*/);       /* LGE_CHANGE : UTS 2012-07-09 jikhwan.jeong@lge.com [L38C][UTS][VDF] Modification for UTS Radio. */
               rsp_ptr->ret = 0;
               break;

       case 1: // UTS Radio Off(PowerDown)
               set_operation_mode(FALSE/*UTS_RADIO_OFF*/);      /* LGE_CHANGE : UTS 2012-07-09 jikhwan.jeong@lge.com [L38C][UTS][VDF] Modification for UTS Radio. */
               rsp_ptr->ret = 1;
               break;
       }
...
testmode_user_table_entry_type testmode_mstr_tbl[TESTMODE_MSTR_TBL_SIZE] =
{
    ...
    /* 11 ~ 20 */
    {TEST_MODE_IRDA_FMRT_FINGER_UIM_TEST,   LGF_TESTIrda,          	          ARM11_PROCESSOR},
    /* 21 ~ 30 */
...
// [LGE_UPDATE_E] minwoo.jung 20120207
//LGE_CHANGE_S mschoi 20130403 add JTAG disable cmd (LG_FW_JTAG_DISABLE)[Start]	
	{TEST_MODE_JTAG_DISABLE,			NULL,							ARM9_PROCESSOR},
//LGE_CHANGE mschoi 20130403	add JTAG disable cmd (LG_FW_JTAG_DISABLE)[End]
...
//LGE_CHANGE jini1711 20120409 add AAT SET diag command [End]    
//LGE_CHANGE chaesun.leem 20130617 add MEID diag command [Start]    
	{TEST_MODE_DLOAD_MEID,                   NULL,                             ARM9_PROCESSOR},
//LGE_CHANGE chaesun.leem 20130617 add MEID diag command [End]      
...
// +e LGBT_DIAG_BT_WIFI_FOR_MFT
    {TEST_MODE_MINIOS_BOOTCHECK,		LGF_TestModeMiniOsBootCheck,	ARM11_PROCESSOR},
    {TEST_MODE_XO_CAL_DATA_COPY,            NULL,                             ARM9_PROCESSOR},
//LGE_CHANGE_S 2012.11 lg-msp@lge.com MTS TEAM
    {TEST_MODE_MTS,							LGF_MTS_OnOff,								ARM11_PROCESSOR},
//LGE_CHANGE_S 2012.11 lg-msp@lge.com MTS TEAM
#if defined(CONFIG_LGE_RCT_41) 	/* bohyun.jung@lge.com	- RCT Porting 4.1 - Bootloader Unlock */
    {TEST_MODE_RCT,							linux_app_handler,								ARM11_PROCESSOR},
#endif 	/* end of CONFIG_LGE_RCT_41 */
};
