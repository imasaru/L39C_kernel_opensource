...
#if 0  // temporary blocking dannydaehee.kim@lge.com 20130425
/* LGE_CHANGE_S [sunho2.park@lge.com] 20130225 : One Cmd for Web download  */
...
extern unsigned char swv_buff[150];
#else
...
void swv_get_func(struct work_struct *work)
{
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	memcpy(&fac_aat_version[0],(&swv_buff[0])+100,50);
	fac_aat_version[49] = '\0';
#endif
	sw_version[49] = '\0';
	fac_version[49] = '\0';
	{
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)	
	{
		...
		if ((ret = call_usermodehelper("/system/bin/setprop", argv, envp, UMH_WAIT_PROC)) != 0) {
			printk(KERN_ERR "%s lge.version.aat set failed to run \": %i\n",__func__, ret);
		}
		else{
			printk(KERN_INFO "%s  lge.version.aat set execute ok\n", __func__);
		}
	}
#endif
	kfree(swv_buffer);
