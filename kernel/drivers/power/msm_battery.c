...
#define BATT_TEMP_CHK_WORK_PERIOD ((HZ)*10)
static struct timer_list batt_timer;
static struct workqueue_struct *local_batt_temper;
static int temper_threshold = 570;
static bool msm_batt_wake_lock_ckeck = false;
static bool msm_batt_timer_check = false;
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
        struct delayed_work batt_work_chk;
        struct wake_lock wake_lock_temper;
#endif
};
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
struct msm_battery_info *batt_info = &msm_batt_info;
#endif
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
        if ((rep_batt_chg.v1.charger_type == CHARGER_TYPE_USB_PC) || (rep_batt_chg.v1.charger_type == CHARGER_TYPE_USB_WALL))
                temper_threshold = 550;
        else
                temper_threshold = 570;

        if ((signed int)rep_batt_chg.v1.battery_temp >= temper_threshold)
        {
                pr_debug("suspend : over %d", temper_threshold);
                if(msm_batt_wake_lock_ckeck == false)
                {
                        pr_debug("temper_chk : wake_lock\n");
                        msm_batt_wake_lock_ckeck=true;
                        wake_lock(&batt_info->wake_lock_temper);
      	                queue_delayed_work(local_batt_temper, &batt_info->batt_work_chk, BATT_TEMP_CHK_WORK_PERIOD);
		}
        }
#endif
...
#if defined (CONFIG_MACH_MSM7X27A_L4II_CDMA)
				else if(((signed int)rep_batt_chg.v1.battery_temp  > 550 ) || ((signed int)rep_batt_chg.v1.battery_temp < -100))
				{
...
#if defined (CONFIG_MACH_MSM7X27A_L4II_CDMA)
				if((((signed int)rep_batt_chg.v1.battery_temp  > 550 ) || ((signed int)rep_batt_chg.v1.battery_temp < -100))
					&& ((msm_batt_info.current_chg_source & AC_CHG) || (msm_batt_info.current_chg_source & USB_CHG)))
				{
					DBG_LIMIT("BATT: Battery -> Not Charging\n");
					msm_batt_info.batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
				}
				else
#endif
...
/* 2013-04-03, jungyeal@lge.com, fixed cut off voltage setting under 3.1V with TA/USB */		
#if defined(CONFIG_MACH_MSM7X25A_V1)
...
#endif
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
static void batt_statue_update_timer(unsigned long ulParam)
{
	if(msm_batt_wake_lock_ckeck == false)
        {
		pr_debug("wake_lock on timer\n");
		msm_batt_wake_lock_ckeck = true;
		wake_lock(&batt_info->wake_lock_temper);
        }
	msm_batt_timer_check = false;
	del_timer(&batt_timer);
        queue_delayed_work(local_batt_temper, &batt_info->batt_work_chk, BATT_TEMP_CHK_WORK_PERIOD);
}

static void init_timer_update(void)
{
        unsigned long dumydata = 0;

        init_timer(&batt_timer);
        batt_timer.expires    = get_jiffies_64() + (HZ*180);
        batt_timer.data         = dumydata;
        batt_timer.function  = batt_statue_update_timer;

        add_timer(&batt_timer);
	msm_batt_timer_check = true;
}

static void msm_batt_temper_chk(struct work_struct *batt_work)
{
	msm_batt_update_psy_status();
	if((signed int)rep_batt_chg.v1.battery_temp > 540)
	{
		pr_debug("temper_chk : over 55\n");
		queue_delayed_work(local_batt_temper, &batt_info->batt_work_chk, BATT_TEMP_CHK_WORK_PERIOD);		
	}
	else
	{
		pr_debug("temper_chk : under 55\n");
		if(msm_batt_wake_lock_ckeck == true)
		{
			pr_debug("temper_chk : wake_unlock\n");
	                if(msm_batt_timer_check == false)
				init_timer_update();
			msm_batt_wake_lock_ckeck = false;
			wake_unlock(&batt_info->wake_lock_temper);
			cancel_delayed_work(&batt_info->batt_work_chk);
		}
	}
}
#endif
...
#if defined(CONFIG_MACH_MSM8X25_V7) || defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	msm_batt_update_psy_status();
#endif
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
        init_timer_update();
        INIT_DELAYED_WORK(&batt_info->batt_work_chk, msm_batt_temper_chk);
	wake_lock_init(&batt_info->wake_lock_temper ,WAKE_LOCK_SUSPEND , "msm_batt_temper_wake_lock");	
#endif
#ifdef CONFIG_HAS_EARLYSUSPEND
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	wake_lock_destroy(&batt_info->wake_lock_temper);
#endif
	return 0;
}
...
static int __init msm_batt_init(void)
{
	...
	pr_debug("%s: enter\n", __func__);
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	local_batt_temper = create_workqueue("msm_batt_temper");
	if(!local_batt_temper)
		return -ENOMEM;
#endif
...
#if defined(CONFIG_MACH_MSM7X27A_L4II_CDMA)
	if(local_batt_temper)
		destroy_workqueue(local_batt_temper);
	local_batt_temper = NULL;
#endif
}
