...
/* LGE_CHANGE_S [sunho2.park@lge.com] 20130103 : One Cmd for Web download  */
...
// L4II_CDMA_TRF_US MUST SUPPORT SHA_HANDSHAKE FEATURE
// DIAG_ERI_CMD_F IS USED FOR AUTHENTICATE TOOLS THAT QUERYING SPC
...
struct delayed_work diag_status_work;

static ssize_t write_diag_status(struct device *dev,
                    struct device_attribute *attr,
                    const char *buf, size_t size)
{
    ...
    if(!strncmp(string, "status", 6))
    {
        schedule_delayed_work(&diag_status_work,msecs_to_jiffies(2000));
#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK_DEBUG
  	    printk("hyoill.leem write_diag_status\n");
#endif
    }
    return size;
}

static DEVICE_ATTR(diag_status, S_IRUGO | S_IWUSR, NULL, write_diag_status);

static void get_diag_status(struct work_struct *data)
{
#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK_DEBUG
	printk("%s \n",__func__);
#endif
...
#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK
	INIT_DELAYED_WORK(&diag_status_work, get_diag_status);
#endif
...
#ifdef CONFIG_LG_FW_USB_ACCESS_LOCK
    ret = device_create_file(&pdev->dev, &dev_attr_diag_status);
    if (ret) {
        printk( KERN_DEBUG "LG_FW : diag device diag_status create fail\n");
        device_remove_file(&pdev->dev, &dev_attr_diag_status);
        return ret;
    }
#endif
