...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		if (other_free < minfree && other_file < lowmem_minfree[i]) {
	...
	if (selected) {
		...
		lowmem_print(1, "Killing '%s' (%d), adj %d, \n"									\
     				...
     				selected->comm, selected->pid,	selected_oom_score_adj,	selected_tasksize * (long)(PAGE_SIZE / 1024),	
     				current->comm, current->pid, other_file * (long)(PAGE_SIZE / 1024),	
					minfree * (long)(PAGE_SIZE / 1024),	min_score_adj,	other_free * (long)(PAGE_SIZE / 1024));
