...
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
int bl_status = 0;
struct lm3530_driver_data *g_drvdata = NULL;
#endif
/* Set to Normal mode */
...
/*0 */   0, 54, 54, 55, 55, 55, 56, 56, 56, 56,
/*1 */  56, 57, 57, 57, 58, 58, 59, 60, 60, 61,
/*2 */  61, 62, 62, 63, 63, 63, 64, 64, 64, 64,
/*3 */  64, 67, 67, 69, 69, 71, 71, 72, 72, 73,
/*4 */  74, 74, 74, 74, 76, 76, 78, 78, 80, 80,
/*5 */  82, 82, 83, 84, 84, 84, 86, 86, 87, 87,
/*6 */  88, 88, 89, 89, 90, 90, 91, 91, 91, 91,
/*7 */  95, 97, 98, 98, 99, 100,101,103,103,103,
/*8 */  104,104,104,104,104,104,104,105,105,105,
/*9 */  106,107,107,107,107,107,107,109,109,110,
/*10 */ 110,111,111,112,112,113,113,113,113,113,
/*11 */ 114,114,115,115,116,116,116,117,117,118,
/*12 */ 118,118,118,120,120,120,121,121,121,122,
/*13 */ 122,123,123,123,123,124,124,124,124,125,
/*14 */ 125,125,125,125,126,127
#endif
};
/*[LGE_CHANGE][kiran.jainapure@lge.com]*/
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
EXPORT_SYMBOL(late_resume_value);
EXPORT_SYMBOL(bl_status);
#endif
/********************************************
 * Functions
 ********************************************/
...
{
	return g_drvdata->state;
}
EXPORT_SYMBOL(lm3530_get_state);
#endif
...
#endif /* CONFIG_PM */
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
void lm3530_bl_control(int status)
{
	if(!bl_chargerlogo)
	{
		if(1 == status)
		{
			printk("%s(), bl_status :%d call lm3530_wakeup func.\n",__func__, bl_status);
			if(late_resume_value == 0) {
			    g_drvdata->intensity = DEFAULT_BRIGHTNESS;
				late_resume_value = DEFAULT_BRIGHTNESS;
			}
			lm3530_wakeup(g_drvdata);
		}
		else
		{
			printk("%s(), bl_status :%d call lm3530_sleep func.\n",__func__, bl_status);
			lm3530_sleep(g_drvdata);
		}
	}
}
EXPORT_SYMBOL(lm3530_bl_control);
#endif
