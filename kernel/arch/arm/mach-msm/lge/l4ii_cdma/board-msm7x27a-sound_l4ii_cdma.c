...
struct pm8xxx_gpio_rpc_cfg pm_mic_mode_param={
	.gpio = PMIC_GPIO_1,
	.config_gpio = true,
	...
	.invert_ext_pin = false,
	.src_pull = PULL_NO_PULL,
	.drv_strength = BUFFER_HIGH,
	.dtest_on = DTEST_DISABLE,
	.ext_config = EXT_PIN_ENABLE,
	.src_config = SOURCE_GND,
	.int_polarity = false,
};
static void l4ii_cdma_pm_set_mic_mode(int state)
{
	static bool init = false;
	static int backup_state = -1;

	if(backup_state == state)
		return;

	if(!init){
		pmic_gpio_config(&pm_mic_mode_param);
		init = true;
	}
...
static void l4ii_cdma_msm_set_mic_mode(int state)
{
#if 1 /*MAX14628 Guide*/
	if(state)
		gpio_direction_output(GPIO_MIC_MODE,1); /*Output-High ==> Mic Bias on*/
	else
		gpio_direction_input(GPIO_MIC_MODE); /*Hi-Z state ==> Mic Bias off*/
#else
	gpio_set_value(GPIO_MIC_MODE, state?1: 0);
#endif
}
static void l4ii_cdma_set_mic_mode(int state)
{
	if(lge_get_board_revno() > LGE_REV_A)
		l4ii_cdma_pm_set_mic_mode(state);
	else
		l4ii_cdma_msm_set_mic_mode(state);
}

static struct gpio_h2w_platform_data l4ii_cdma_h2w_data = {
	.gpio_detect = GPIO_EAR_SENSE,
	.gpio_button_detect = GPIO_BUTTON_DETECT,
	.set_mic_mode = l4ii_cdma_set_mic_mode,
};
...
void __init lge_add_sound_devices(void)
{
	...
	rc = gpio_request(GPIO_MIC_MODE, "h2w_mic_mode");
	if (rc) {
		printk(KERN_ERR "%d gpio request is failed\n", GPIO_MIC_MODE);
	}
	else{
		gpio_tlmm_config(GPIO_CFG(GPIO_MIC_MODE, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);	
	}
	
	//Rev.B Only
	rc = gpio_request(GPIO_EAR_DETECT_EN, "ear_detect_en");
	if (rc) {
		printk(KERN_ERR "%d gpio request is failed\n", GPIO_EAR_DETECT_EN);
	} else {
		gpio_tlmm_config(GPIO_CFG(GPIO_EAR_DETECT_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}
	gpio_set_value(GPIO_EAR_DETECT_EN,1);
#endif
