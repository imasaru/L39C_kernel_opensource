...
/*LGE_CHANGE_S :  Dump androind logs to console.
By enabling UART & making logger mask to one of below logs can sent through uart
when adb is not up */
...
#endif
/*LGE_CHANGE_E : Dump androind logs to console */
...
#ifdef CONFIG_MACH_MSM7X27A_L4II_CDMA
    if(!strcmp(battery_id, "ds2704_n"))
        lge_battery_info = 1;
    else if(!strcmp(battery_id, "ds2704_l"))
        lge_battery_info = 1;
    else if(!strcmp(battery_id, "isl6296_n"))
        lge_battery_info = 1;
    else if(!strcmp(battery_id, "isl6296_l"))
        lge_battery_info = 1;
    else if(!strcmp(battery_id, "isl6296_c"))
        lge_battery_info = 1;
    else if(!strcmp(battery_id, "ds2704_c"))
        lge_battery_info = 1;
	else
		lge_battery_info = 0;
#endif
