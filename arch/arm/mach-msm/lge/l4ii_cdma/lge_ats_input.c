...
#define DRIVER_NAME "lge_ats_input"
					
extern int touch_get_x_max (void);
extern int touch_get_y_max(void);
extern int touch_get_z_max(void);
extern int touch_get_max(void);
...
static int  __devinit ats_input_probe(struct platform_device *pdev)
{
	...
	ats_input_dev->name = DRIVER_NAME;

	//for(i=0; i<EV_CNT; i++)
	//	set_bit(i, ats_input_dev->evbit);
	set_bit(EV_SYN, ats_input_dev->evbit);
	set_bit(EV_KEY, ats_input_dev->evbit);
	set_bit(EV_REL, ats_input_dev->evbit);
	set_bit(EV_ABS, ats_input_dev->evbit);
	set_bit(EV_PWR, ats_input_dev->evbit);
	for(i=0; i<KEY_CNT; i++)
	...
	input_set_abs_params(ats_input_dev, ABS_MT_TOUCH_MAJOR, 0, touch_get_z_max(), 0, 0);
	input_set_abs_params(ats_input_dev, ABS_MT_TRACKING_ID, 0, touch_get_max()-1, 0, 0);
	input_set_abs_params(ats_input_dev, ABS_MT_PRESSURE, 0, 255, 0, 0);
...
static int __devexit ats_input_remove(struct platform_device *pdev)
{
...
static int __devinit ats_input_init(void)
{
	return platform_driver_register(&ats_input_driver);
}


static void __devexit ats_input_exit(void)
{
