...
#ifdef BT_SW_I2C 
#define BT_GPIO_I2C_SDA   10
#define BT_GPIO_I2C_SCL   9
#endif
...
/* LGE_S vedhachalam.krishnan@lge.com
         BT QUP I2C converted to Software I2c
         for touch performance
         gpio pins 131,132 -> 9,10 */
#ifdef BT_SW_I2C
static struct gpio_i2c_pin bt_i2c_pin[] = {
	[0] = {
		.sda_pin	= BT_GPIO_I2C_SDA,
		.scl_pin	= BT_GPIO_I2C_SCL,
		.reset_pin	= 0,
	},
};

static struct i2c_gpio_platform_data bt_i2c_pdata = {
	.sda_is_open_drain	= 0,
	.scl_is_open_drain	= 0,
	.udelay			= 1,
};

static struct platform_device bt_i2c_device = {
	.name	= "i2c-gpio",
	.dev.platform_data = &bt_i2c_pdata,
};


static int init_gpio_i2c_pin_bt(
	...
	struct i2c_board_info *i2c_board_info_data)
{
	...
	gpio_request(BT_GPIO_I2C_SDA, 	"BT_I2C_SDA");
	gpio_request(BT_GPIO_I2C_SCL, 	"BT_I2C_SCL");
	...
	gpio_free(BT_GPIO_I2C_SDA);
	gpio_free(BT_GPIO_I2C_SCL);

	return 0;
}


static void __init marimba_init_i2c_bt(int bus_num)
{
	//	bus_num = 121;
	bt_i2c_device.id = bus_num;

	init_gpio_i2c_pin_bt(&bt_i2c_pdata, bt_i2c_pin[0], bahama_devices);
	i2c_register_board_info(bus_num, bahama_devices, ARRAY_SIZE(bahama_devices));
	platform_device_register(&bt_i2c_device);


	printk("into bt_init_i2c\n");
}
#endif
/* LGE_E */
...
/* LGE_S vedhachalam.krishnan@lge.com
	...
	/* Rev A and Rev B target uses HW i2c for BT, from Rev C SW i2c will be used for BT */
	if(hw_revision == LGE_REV_A)
	{
	    marimba_pdata.scl = 131;
		marimba_pdata.sda = 132;
		...
		lge_add_gpio_i2c_device(marimba_init_i2c_bt);
