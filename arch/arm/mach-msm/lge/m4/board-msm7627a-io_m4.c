...
/* 2013-03-08 linmao.li@lge.com [M4DS] Single HOME Key Bring Up [START] */ 
#if (defined(CONFIG_MACH_MSM7X25A_M4DS)) /*kiran.jainapure@lge.com: For M4 DS*/
...
#endif
/* 2013-03-08 linmao.li@lge.com [M4DS] Single HOME Key Bring Up [END] */ 
...
/*kiran.jainapure@lge.com: For M4 DS*/
#if (defined(CONFIG_MACH_MSM7X25A_M4DS))
void touch_init(void)
{
	int rc;
	
	/* gpio init */
	rc = gpio_request(TS_TOUCH_LDO_EN, "TOUCH_PANEL_PWR");

	gpio_tlmm_config(
		GPIO_CFG(TS_TOUCH_LDO_EN, 0, GPIO_CFG_OUTPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	
	gpio_free(TS_TOUCH_LDO_EN);
}
#endif
/*kiran.jainapure@lge.com: For M4 DS*/
...
/*kiran.jainapure@lge.com: For M4 DS*/
		...
		if (onoff){
			...
			rc = gpio_direction_output(TS_TOUCH_LDO_EN, 1);
		}
		else{
			printk("[Melfas]touch power off\n");
			rc = gpio_direction_output(TS_TOUCH_LDO_EN, 0);
		}	
